...
To simulate real-world attack scenarios and test the resilience of the network infrastructure, various network-level attacks were carried out using a developed Python script, [`attack-tool.py`](./attack-tool.py), based on the **Scapy** library.

The script performs several **reconnaissance** and **Denial-of-Service (DoS)** attacks as well as **Man-in-the-Middle (MitM)** techniques. These attacks were launched from both the internal and external attacker nodes and are intended to evaluate the effectiveness of security mechanisms such as **Snort**, **Splunk**, and the **pfSense firewall**.

The implemented attack functionalities include:

- **Ping Sweep**: Identifies active hosts in a given subnet by sending ICMP Echo Requests.
- **TCP SYN Port Scan**: Scans for open TCP ports on a target host using half-open SYN scanning.
- **ICMP Flood**: Sends a high volume of ICMP packets to overload a target system.
- **TCP SYN Flood**: Sends TCP SYN packets with spoofed source IPs to exhaust server resources.
- **UDP Flood**: Sends UDP packets to a specific port in high volume, aiming to consume bandwidth and resources.
- **ARP & DNS Spoofing**: Performs ARP poisoning to intercept traffic between two hosts and optionally spoofs **DNS** responses to redirect domain queries.

All attack modules can be configured with customizable parameters, such as target IP, port range, packet interval, and packet count. The script features a command-line interface that allows easy selection of attack types and corresponding configurations.

These simulated attacks provide a practical framework for verifying the detection capabilities of deployed security tools and help in identifying potential weaknesses in network segmentation, filtering rules, and monitoring systems.

### Running the Attack Tool

To execute the attack script, navigate to the repository directory and run the Python file using the following command:

```
sudo python3 attack-tool.py
```

Make sure you have the required dependencies installed. The script relies on the Scapy library. You can install it using:
